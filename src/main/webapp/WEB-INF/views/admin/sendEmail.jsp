...
function sendFile(file, editor) {
    // 파일 전송을 위한 폼생성
	    ...
	    $.ajax({ // ajax를 통해 파일 업로드 처리
            ...
            // 에디터에 이미지 출력
	        	jQuery(editor).summernote('editor.insertImage', "http://${myIP}${root}/upload"+data.url);
	        }
	    });
	}
</script>
...
<body>
	<div style="width: 800px; margin: 0 auto;">
		<form style="display: hidden"
			action="http://${myIP}/animalshelter/sendEmail.animal" method="POST"
			id="form">
			...
			</select> <input id="emailSubject" type="text" placeholder="이메일 제목을 입력해주세요"
				value="" size="80"><br> <br>
		...
		jQuery('#summernote').summernote({
			...
			<input type="button" id="btnSend" value="전송하기"> <input
				type="hidden" id="content" name="content" value="" /> <input
				type="hidden" id="receiver" name="receiver" value="" /> <input
				type="hidden" id="title" name="title" value="" />
		</form>
	</div>
</body>
</html>